...
def get_dataset_metrics(true_labels, pred_labels):
    ...
    for true_label, pred_label in zip(true_labels, pred_labels):
        # Initialize metrics for true_label if not present
        if true_label not in metrics_dict:
            metrics_dict[true_label] = {"TP": 0, "FP": 0, "FN": 0, "Support": 0}
        
        # Initialize metrics for pred_label if it's not 'OTHER'
        if pred_label != "OTHER" and pred_label not in metrics_dict:
    ...
    for field in metrics_dict:
        ...
        if field != "OTHER":
            # Create a temporary DataFrame for the current field
            ...
            # Append temp_df to df using pd.concat
...
if __name__ == "__main__":
    ...
    doc_true = f"{os.getcwd()}\\dataset\\val\\boxes_transcripts"
    
    doc_pred = f"{os.getcwd()}\\dataset\\val_w_ann\\boxes_transcripts_labels"
